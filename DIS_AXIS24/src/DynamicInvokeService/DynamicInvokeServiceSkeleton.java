...
import zc.ws.dynamicinvokeservice.axis.services.dynamicinvokeservice.FaultType;
import zc.ws.dynamicinvokeservice.axis.services.dynamicinvokeservice.GetDynamicInvokeFault;
 ...
 */
public class DynamicInvokeServiceSkeleton implements
		DynamicInvokeServiceSkeletonInterface {
	/**
	 * Auto generated method signature
	 * 
	 * @param getDynamicInvokeRequest0
	 * @return getDynamicInvokeResponse1
	 * @throws GetDynamicInvokeFaultException
	 */
	public void myException() {// 封装Fault示例
		GetDynamicInvokeFault gif = new GetDynamicInvokeFault();
		FaultType param = new FaultType();
		param.setServiceName("1");
		param.setOperationName("1");
		param.setExceptionReason("1");
		param.setExceptionCode("1");
		gif.setFaultName(param);
		// FaultType param1=new FaultType("1","1","1","1");//没有构造函数
	}
	...
	public static void test4() {// Axis2 \ emptyInput 测试
		...
		/*
		 * ParameterType[] parameters1 = new ParameterType[2]; parameters1[0] =
		 * new ParameterType(); parameters1[0].setName("routerName"); String
		 * value1 =
		 * "<routerName xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:type=\"tes:cRouterType\"><router xsi:type=\"tes:routerType\"><serviceName xsi:type=\"xsd:string\">1</serviceName><operationName xsi:type=\"xsd:string\">2</operationName></router><terminal xsi:type=\"tes:terminalType\"><name xsi:type=\"xsd:string\">3</name><value xsi:type=\"xsd:string\">4</value><link xsi:type=\"tes:linkType\"><name xsi:type=\"xsd:string\">5</name><value xsi:type=\"xsd:string\">6</value></link></terminal></routerName>"
		 * ; parameters1[0].setValue(value1); parameters1[1] = new
		 * ParameterType(); parameters1[1].setName("linkName"); String value2 =
		 * "<linkName xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:type=\"tes:cLinkType\"><link xsi:type=\"tes:linkType\"><name xsi:type=\"xsd:string\">7</name><value xsi:type=\"xsd:string\">8</value></link><value xsi:type=\"xsd:string\">9</value></linkName>"
		 * ; parameters1[1].setValue(value2);
		 * parameterList.setParameter(parameters1);
		 */
	...
	public static void test5() {// Axis2 \ emptyInput 测试 包含异常部分
		// 数据封装
		GetDynamicInvokeResponse response = new GetDynamicInvokeResponse();
		ResultListType result = new ResultListType();
		ExceptionListType exception = new ExceptionListType();
		...
		try {
			...
			sender.setOptions(options);
			System.out.println("input" + input);
			output = sender.sendReceive(input);// 此处发生调用的异常！
			paramR[0] = output.toString();
			result.setResult(paramR);
			response.setResultList(result);
			System.out.println(response.getResultList().getResult()[0]
			...
			e.printStackTrace();
			paramE[0] = e.getReason();// 返回异常原因：Connection refused: connect
			// System.out.println(e.get);
			// return; // 是由于服务没有开启
			// System.out.println(paramE[0]);
			exception.setException(paramE);
			response.setExceptionList(exception);
			System.out.println(response.getExceptionList().getException()[0]
					+ "***********");
	...
	public static void test6() {// Axis2 \ emptyInput 测试 包含异常部分
		// 数据封装
		GetDynamicInvokeResponse response = new GetDynamicInvokeResponse();
		ResultListType result = new ResultListType();
		ExceptionListType exception = new ExceptionListType();
		...
		DynamicInvokeServiceSkeleton dis = new DynamicInvokeServiceSkeleton();
		ParameterListType parameterList = null;
		String serviceName = "ExceptionTestService";
		String operationName = "singleInput";
		ParameterType[] parameters1 = new ParameterType[2];
		parameters1[0] = new ParameterType();
		parameters1[0].setName("routerName");
	...
	public OMElement String2Axiom(String serviceName, String operationName,
		...
		// "http://Router_DataService.ws.zc:8080/axis/services/Router_DataService/";//
		// 以后需要通过serviceName来获取命名空间。//将它改为一个函数
		String targetNamespace = "http://ExceptionTestService.ws.zc:8080/axis/services/ExceptionTestService/";// 从什么地方获取？自己写一个解析wsdl的服务；
		OMNamespace omNs = fac.createOMNamespace(targetNamespace, "ns1");
		OMElement method1 = fac.createOMElement(operationName, omNs);
		// 如果参数非空，需要挂载子节点；
